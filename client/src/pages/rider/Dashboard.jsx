...
export default function RiderDashboard() {
  ...
  const [showConfirm,   setShowConfirm]   = useState(false) // cancel confirmation
  ...
  // ── poll active ride every 5 s — this is how OTP stays visible ─
  useEffect(() => {
    const interval = setInterval(checkActiveRide, 5000)
    return () => clearInterval(interval)
  }, [])

  // ── auto-switch to active tab when driver accepts ─────────
  useEffect(() => {
    if (activeRide && ['accepted','in_progress'].includes(activeRide.status)) {
      // only auto-switch if rider is on home/book tab (not history/profile)
  ...
  const checkActiveRide = async () => {
    try {
      ...
      if (r.data.ride) {
        setActiveRide(r.data.ride)   // OTP comes from server every poll
      } else {
        // ride ended (completed/cancelled) — refresh history
        if (activeRide) {
          loadHistory()
        }
        setActiveRide(null)
  ...
  const requestRide = async (e) => {
    ...
    try {
      ...
      // immediately fetch active ride so OTP is loaded from server
  ...
  // ── cancel with two-step confirmation ────────────────────
        ...
        <div style={S.content}>
          ...
          {tab === 'home' && (
            <>
              ...
              {/* Active ride banner on home tab */}
              {hasActiveRide && (
                <div style={{
                  ...
                  <div style={S.cardBody}>
                    {hasActiveRide ? (
                      <div style={{
                        ...
                        color:'#8b93a8', fontSize:13
                      }}>
                        <div style={{fontSize:28, marginBottom:12}}>🚖</div>
                        You already have an active ride.
                        <br/>
                        <button style={{
                          ...S.btn, marginTop:16, maxWidth:200
                        }} onClick={() => setTab('active')}>
                          View Active Ride
                        </button>
                      </div>
                ...
                <>
                  <div style={S.card}>
                    ...
                    <div style={S.cardBody}>
                      ...
                      {/* ── OTP — ALWAYS VISIBLE, from server poll ── */}
                      <div style={{
                      ...
                      {/* ── CANCEL — only allowed before ride starts ── */}
